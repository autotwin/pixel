...
def create_skull_still(
    stl_path_file: Path, vis_path: Path, elev: float = -90, azim: float = 90
):
    """"""
    ...
    ax.add_collection3d(
        mplot3d.art3d.Poly3DCollection(
            imported_mesh.vectors, alpha=1.0, ec="k", fc=(0.75, 0.75, 0.75)
        )
    )  # (0.75,0.75,0.75)
...
def run_visualization_code(input_file_str: str) -> None:
    ...
    vis_path, stl_path_file_outer = get_visualization_relevant_path_names(
        input_file_str
    )
